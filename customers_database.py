...
class User:
    def __init__(self, useraccount, username, balance):
        self.useraccount = useraccount
        self.name = username
        self.balance = balance
        print(
            "{}, You created a bank account, your account number is {} with {} dollars".format(
                self.username, self.useraccount, self.balance
            )
        )

    def receive_internal_transfer(self, from_account, amount):
        self.balance += amount
        print(
            "Receive {} from {}. Your balance now is {}".format(
                amount, from_account, self.balance
            )
        )

    def __repr__(self):
        return "{} - {} - {}".format(self.useraccount, self.username, self.balance)

    def __str__(self):
        return self.__repr__()


user1 = User("23579999", "ycheryl", 20)
user2 = User("23579998", "howard123", 50)
user3 = User("23579997", "dilys789", 40)
user4 = User("23579996", "andrew217", 70)
user5 = User("23579995", "andy05", 100)
user6 = User("23579994", "david07", 15)

user1.receive_internal_transfer("23579998", 30)
print(user1.balance)
print(user1)
