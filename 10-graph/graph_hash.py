class Graph:
    ...
    def BFS(self, v):
        self.bfs = {
            i: {"color": "W", "distance": None, "predecessor": None} for i in self.data
        }
        self.bfs[v]["color"] = "G"
    ...
    - Keep track of the predecessor of vertices
    - Marks 2 auto-incrementing timestamps discovery and finish to indicate when the node
    ...
    def DFS(self, verbose=False):
        self.dfs = {
            v: {"color": "W", "predecessor": None, "discovery": None, "finish": None}
            for v in self.data
        }
        time = 0
        for v in self.data:
...
def simple_test():
    for test in simple_tests:
        ...
        print(
            "Test 7: BFS - Shortest path from {} to {}: ".format(
                G.vertices[0], G.vertices[end]
            )
        )
